...
parser.add_argument("--measurements","-m", help="Done measurements AST file", required=True)
...
all_probes_dict = literal_eval(open("data/" + "probe_" + measurements_split[1] + "_" + measurements_split[2]).read())
done_measurements = literal_eval(open("data/" + args.measurements).read())

output = []

for measurement in done_measurements:
	r = requests.get(atlas_site + '/api/v1/measurement/' + str(measurement) + '/result/?key=' + atlas_download_api_key )
...
for area in areas:
	...
	result = pr.kd_tree.resample_gauss(swath_def, data, area_def, radius_of_influence=300000, sigmas=pr.utils.fwhm2sigma(300000),
    fill_value=None)
	pr.plot.save_quicklook("output/" + target + time + area + '_map.png', area_def, result, num_meridians=0, num_parallels=0, label='Latency (ms)')
