...
parser.add_argument("--addressfamily","-a", help="Address Family to use, default is 4", type=int, choices=[4,6], default=4)
parser.add_argument("--numberofpkts","-", help="Number of packets to send default is 16", type=int, choices=range(1,17), default=16)
parser.add_argument("--public", help="Set to make public measurements", action="store_true", default=False)
parser.add_argument("--resolveonprobe", help="Set to resolve DNS on probes", action="store_true", default=False)
...
r = requests.get(atlas_site + "/api/v1/probe/?limit=100&status=1" + tags + filters)
...
#save probes state
open("data/probe_" + args.target + "_" + now.strftime("%Y%m%dT%H%M") + ".ast", "w").write(repr(all_probes_dict))
...
open("data/measurements_" + args.target + "_" + now.strftime("%Y%m%dT%H%M") + ".ast", "w").write(repr(done_measurements))
